...
//需要输出的文件目录，从项目根目录开始
const outputFilePath = `./dist/unittest/mul.txt`;
//输出目录
...
function testadd() {
  ...
  let result = String(BigNumber(oneValue).times(twoValue));
  let countreulst = mul(oneValue, twoValue);
  let nowstr = `simplecount:mul(${oneValue},${twoValue})=${countreulst}`;
  // 1,100 -> 1100
