//Done in 388.07s.
//oneValue=-110,twoValue=-110,endValue=130,step=0.0001,Done in 44.98s.
...
//需要输出的文件目录，从项目根目录开始
const outputFilePath = `./dist/unittest/sub.txt`;
//输出目录
...
function testadd() {
  ...
  let result = String(BigNumber(oneValue).minus(twoValue));
  let countreulst = sub(oneValue, twoValue);
  let nowstr = `simplecount:sub(${oneValue},${twoValue})=${countreulst}`;
  // 1,100 -> 1100
